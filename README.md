...
Cronify is a small utility that translates natural-language into cron entries.

```bash
cronify "weekdays at 07:15" --comment "Morning sync" --dry-run -- ./sync.sh
```
...
cargo install --path .
cronify "every 30 minutes" --dry-run -- env COMMAND=backup /usr/local/bin/backup.sh
```
...
cronify "daily at 02:00" --comment "Nightly backup" --dry-run -- tar -czf /backups/site.tar.gz /var/www

# Write to cron with environment variables
cronify "weekly on fri at 03:30" --file /etc/cron.d/weekly-maint --env PATH=/usr/local/bin --env RUST_LOG=info -- systemctl restart my-service

# JSON output
cronify --json "every 10 minutes" -- echo "tick"
```

## Supported phrasing

Run `cronify --list-patterns` to see accepted shapes and examples.
